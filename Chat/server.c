...
void *thread_function(void *arg) {
    int i;
    printf("Command list: help, user, chat, ip\n");
    while (1) {
        ...
        if (!strcmp(bufmsg, "\n")) continue;
            ...
            printf("help, user, chat, ip\n");
        else if (!strcmp(bufmsg, "user\n"))
            printf("Number of current users = %d\n", num_user);
        else if (!strcmp(bufmsg, "chat\n"))
            printf("Total number of conversations = %d\n", num_chat);
        else if (!strcmp(bufmsg, "ip\n"))
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (FD_ISSET(listen_sock, &read_fds)) {
            ...
            printf("User added. Current number of users = %d\n", num_user);
...
void removeClient(int s) {
    ...
    fprintf(stderr, "\033[33m");
    printf("[%02d:%02d:%02d]", tm.tm_hour, tm.tm_min, tm.tm_sec);
    printf("A user left. Current number of users = %d\n", num_user);
    fprintf(stderr, "\033[32m");
    fprintf(stderr, "server>");
}
